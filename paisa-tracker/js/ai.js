/**
 * ai.js — AI Chat + Smart Local Parser (OpenAI Version)
 * Hybrid Mode:
 * - Simple expenses → handled locally (no API needed)
 * - Complex questions → sent to OpenAI via /api/chat
 */
...
function initAIChat() {
  addAIMessage(
    ...
    `⚡ Basic features work instantly using **Smart Local Mode**.\n\n` +
    `Try:\n` +
    `- *"spent 150 on mess today"*\n` +
    `- *"chai 30, auto 60, lunch 120"*\n` +
    `- *"set budget to 5000"*\n` +
    `- *"how much have I spent?"*\n\n` +
    `For advanced insights, I’ll use AI automatically.`
  );
}
